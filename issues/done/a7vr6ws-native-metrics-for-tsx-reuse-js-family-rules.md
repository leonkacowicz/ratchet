...
- **Two rca bugs found, reproduced, then dropped:** the naming fallback never fires (ids 153/236
  are `DASHQMARKCOLON`/`ClassHeritage` in the TSX grammar), and — unique to TSX —
  ...
  nesting increment (caught by `example.tsx`: cognitive 4 vs 2). Parity was proved against both,
  then dropped — TSX now names anonymous functions and scores `else if` exactly as TS and JS do,
  and all three share one `JS_FAMILY` rule set. Pinned by golden tests; file-level metrics keep
  rca parity.
